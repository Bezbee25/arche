...
arche task run
  └── reads  spec.md + archi.md + done tasks + session log
  └── calls  claude / gemini with the full context
  └── writes architecture decisions → archi.md
...
Every `arche task run` assembles a prompt from:

| Source | Content |
|--------|---------|
| `spec.md` | Track goal and requirements |
| `archi.md` | Architecture memory — grows automatically after each run |
| Done tasks | What was completed (with notes) |
| Pending tasks | What comes next |
| Session log | Recent session entries (last ~1500 chars) |

After each run, the LLM response is scanned for an **"Architecture notes"** section. If found, it is appended to `archi.md`. This is what makes context accumulate — each task leaves a trace for the next one.
...
└── tracks/
    └── {track-id}/
        ...
        ├── archi.md          ← architecture memory (auto-updated)
